...
한 이미지에 4096 차원의 특징 벡터들이 존재하는데 각 이미지 마다 특징 벡터들을 Nearest Neighbor Algorithm을 돌리게 되면,  
...
4096차원의 특징 벡터를 PCA Algorithm이나 t-SNE Algorithm으로 2차원으로 차원 축소를 하게 되면  
...
저 예시는 MNIST dataset을 가지고 2차원으로 군집화한 것을 나타낸 것이다.  
...
어떤 이미지가 들어와야 각 뉴런들의 활성화가 최대화가 되는지 알아볼 수 있지 않을까?  
...
더 나아가서 **Saliency Maps**라는 연구도 있는데  
이는 어떤 픽셀을 보고 이미지를 분류했는지 알아보는 연구이다.  
