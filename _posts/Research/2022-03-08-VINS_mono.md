...
$b_1, b_2, \hat{g}$는 첫번째 camera frame에서의 좌표계를 나타낸 것이라 이해하면 되는데(주관적인 해석),  아래의 식 (17)을 보면 알겠지만 $[b_k,b_{k+1}]$ 사이에 gravity term$(g^{c0})$이 존재한다.  
...
Window 첫번째 frame부터 마지막 frame까지 가지고 있는 모든 gravity term$(g^{c0})$를 사용하면서 $\hat{g}$가 $[0,0,1]$로 수렴할 때까지 알고리즘을 수행한다.  
